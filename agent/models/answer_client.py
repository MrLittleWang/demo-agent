...
def compose_grounded_answer(*, user_message: str, deterministic_answer: str,
                            intent_result: Any) -> GroundedAnswerResult:
    """让模型基于结构化意图生成克制客服回复；失败时回退到课程边界话术。"""
    if os.getenv("AGENT_COURSE_DISABLE_LLM") == "1":
        return GroundedAnswerResult(answer=deterministic_answer,
                                    fallback_reason="test_model_disabled")
    api_key = os.getenv("ANTHROPIC_API_KEY")
    base_url = os.getenv("ANTHROPIC_BASE_URL", "https://api.siliconflow.cn/v1")
    ...
    payload = {
        ...
        if hasattr(intent_result, "model_dump") else intent_result,
        "fallback_answer":
        deterministic_answer,
        "lesson_boundary":
        "当前只完成粗意图识别，不能承诺订单、规则、退款、赔偿或人工流转结果。",
    }
    messages = [
        {
            "role":
    ...
    try:
        ...
        return GroundedAnswerResult(answer=answer,
                                    used_model=True,
                                    model_name=model)
