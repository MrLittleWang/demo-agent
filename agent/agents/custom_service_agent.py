...
INTENT_RULES: list[tuple[Intent, list[str], str]] = [
    ("complaint", ["投诉", "举报", "赔偿", "曝光", "315",
                   "别踢皮球"], "用户表达了投诉、赔偿或强烈不满，规则高置信标记为投诉类消息。"),
    ("refund_request", ["退款", "退货", "取消订单", "坏了", "无法开机",
                        "质量问题"], "用户在询问退款、退货或质量问题，规则高置信标记为售后退款类消息。"),
    ("order_query", ["订单", "物流", "快递", "发货", "到哪",
                     "运单"], "用户在询问订单或物流状态，规则高置信标记为订单查询类消息。"),
    ("promotion_consult", ["优惠", "活动", "会员价", "券", "满减",
                           "折扣"], "用户在询问优惠或活动，规则高置信标记为活动咨询类消息。"),
    ("product_consult", ["耳机", "充电器", "音箱", "推荐",
                         "哪个好"], "用户在询问商品或推荐，规则高置信标记为商品咨询类消息。"),
...
def build_rule_evidence(
        message: str) -> list[tuple[Intent, list[str], list[str], str]]:
    ...
    for intent, keywords, explanation in INTENT_RULES:
        ...
        negated = [
            ...
            if is_negated_keyword(message, keyword)
        ]
...
def plan_intent_by_rules(user_message: str) -> IntentResult | None:
    ...
    negated_keywords = [
        keyword for _, _, negated, _ in evidence for keyword in negated
    ]

    # 明确投诉仍按课程既有顺序优先；它只是分拣信号，不代表已经转人工或批准赔偿。
    complaint = next(
        (item for item in active_evidence if item[0] == "complaint"), None)
    if complaint:
        return IntentResult(
            intent="complaint",
            source="rules",
    ...
    refund = next(
        (item for item in active_evidence if item[0] == "refund_request"),
        None)
    ...
    core_evidence = [
        ...
        if any(keyword not in CONTEXT_KEYWORDS.get(item[0], set())
    ...
    if len(core_evidence) > 1 or (not core_evidence and len(active_evidence)
                                  > 1) or negated_keywords:
        primary = core_evidence[0] if core_evidence else active_evidence[0]
        intents = "、".join(item[0]
                           for item in core_evidence or active_evidence)
        return IntentResult(
            intent=primary[0],
            source="rules",
            confidence=0.65,
...
def build_customer_msssage(request: ChatRequest) -> list[dict[str, str]]:
    ...
    system_message = ("你是小杰电商公司的第一版 AI 客服。请用自然、耐心的客服语气回答用户问题。"
...
class CustomServiceAgent:
    ...
    def chat(self, request) -> ChatResponse:
        ...
        self._message_count_by_session[
            request.session_id] = self._message_count_by_session.get(
        ...
        log_course_event("INTENT_CLASSIFIED",
                         "结构化意图识别完成",
                         teaching=True,
        ...
        log_course_event("SAFE_ANSWER_BOUNDARY_READY",
                         "确定性回答边界已生成",
        ...
        log_course_event("GROUNDED_ANSWER_READY",
                         "受边界约束的模型回答已生成",
                         teaching=True,
