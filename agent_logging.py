...
def _decorate_decorate_operation(func: Callable[..., Any], operation_name: str) -> Callable[..., Any]:
    lession_id = _CURRENT_LESSON_ID
    if inspect.iscoroutinefunction(func):
        @wraps(func)
        async def wrapper(*args: Any, **kwargs: Any)-> Any:
            token, model_token, teaching_token, started = _begin(args, kwargs, operation_name, lession_id)
            try:
        ...
        def wrapper(*args: Any, **kwargs: Any)-> Any:
            ...
            try:
                ...
                _TEACHING_EVENT_COUNT.reset(teaching_token)
    return wrapper
def _begin(args: tuple[Any, ...], kwargs: dict[str, Any], operation_name: str, lession_id: str):
    request = _reqest(args, kwargs)
    token = _LOG_CONTEXT.set((lession_id, str(getattr(request, "session_id", "-"))))
    ...
    logger = logging.getLogger("xiaojie_agent")
...
def _reqest(args: tuple[Any, ...], kwargs: dict[str, Any])->Any|None:
    return next((item for item in (args, kwargs.values()) if hasattr(item, "session_id")), None)
